...
$$\begin{aligned} 	\sem{\varphi_1 \starts \varphi_2}(\Stream) 	&= \{C_1 \cup C_2 \mid C_1 \in \sem{\varphi_1}(\Stream)  	\land C_2 \in \sem{\varphi_2}(\Stream) \\ 	&\quad \land \cstart(C_1) = \cstart(C_2) \land \cend(C_1) \leq \cend(C_2)\} \end{aligned}$$
...
\begin{theorem} \label{theo:starts}
...
\begin{proof}
	...
	$$\begin{aligned}[t] 		
		& \Qstarts = Q_2 \uplus (Q_1 \times Q_2 \times \{0, 1\}) \\ 		
		& \Pstarts = \SETP_2 \cup \{ P_1 \land P_2 \mid P_1 \in \SETP_1 \land P_2 \in \SETP_2 \} \hspace*{155px}\\ 		
		& \Xstarts  = \SETX_1 \cup \SETX_2 \\ 		
		& I_{\starts} = (q_0, p_0, 0) \text{ where } q_0 \text{ and } p_0 \text{ are the initial states of } \cA_{\varphi_1} \text{ and } \cA_{\varphi_2}.\\ 		
		& F_{\starts} = F_2 \text{ the final states of } \cA_{\varphi_2}. 	\end{aligned}$$
	...
	$$\begin{aligned}[t] 		
		...
		& \uplus \{((q, p, 1), \epsilon, p) \mid q \in F_1 \land p \in Q_2\} 	\end{aligned}$$
