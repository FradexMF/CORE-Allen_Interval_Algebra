...
$$\begin{aligned} 	\sem{\varphi_1 \during \varphi_2}(\Stream) 	&= \{C_1 \cup C_2 \mid C_1 \in \sem{\varphi_1}(\Stream)  	\land C_2 \in \sem{\varphi_2}(\Stream) \\ 	&\quad \land \cstart(C_1) \leq \cstart(C_2) \land \cend(C_2) \leq \cend(C_1)\} \end{aligned}$$
...
\begin{proof}
	...
	$$\begin{aligned}[t] 		 		
		& \Qduring = (Q_1 \times \{0\}) \uplus (Q_1 \times Q_2 \times \{0, 1\}) \uplus (Q_1 \times \{2\}) \\ 		 		
		...
		& F_{\during} = \{ (q, 2) \mid q \in F_1 \}. 	 	\end{aligned}$$
	
	The transition relation $\Dduring$ is formally defined as:
	
	$$\begin{aligned}[t] 		 		
		\Dduring 
		& = \{((q,b), \epsilon, (q',b)) \mid (q,\epsilon,q') \in \Delta_1 \land b \in \{0, 2\}\} \\ 		 		
		& \uplus \{((q,b), P_1, L_1, (q', b)) \mid (q,P_1, L_1,q') \in \Delta_1 \land b \in \{0, 2\}\} \\ 		 		
		& \uplus \{((q,0), \epsilon, (q, p_0, 0)) \mid q \in Q_1 \} \\ 		 		
		...
		& \uplus \{((q, p, 1), \epsilon, (q, 2)) \mid p \in F_2 \land q \in Q_1\} 	 	\end{aligned}$$
	
	Intuitively, given a stream $\Stream$, $\cAduring$ first evaluates $\varphi_1$ alone. Because $\cstart(C_1) \leq \cstart(C_2)$, it can non-deterministically transition to evaluate both simultaneously at any point. Once $\cA_{\varphi_2}$ reaches a final state (having consumed at least one joint physical event, tracked by the bit $1$), we jump back to evaluating only $\varphi_1$. Since $\cend(C_2) \leq \cend(C_1)$, $\cA_{\varphi_1}$ can either accept immediately if it is already in a final state, or continue consuming events before finishing.
